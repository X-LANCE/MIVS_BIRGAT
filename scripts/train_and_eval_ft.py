...
if args.read_model_path: # load from checkpoints or testing mode
    ...
    params.fine_tuning, params.few_shot = args.fine_tuning, args.few_shot
    if params.fine_tuning:
        params.lr, params.max_iter, params.eval_after_iter = args.lr, args.max_iter, args.eval_after_iter
    args = params
...
if not os.path.exists(os.path.join(exp_path, 'params.json')):
...
if not args.testing:
    ...
    if args.fine_tuning: files = args.tgt_files if args.tgt_files else args.files
    train_dataset = Example.load_dataset('train', files, args.domains, fine_tuning=args.fine_tuning, few_shot=args.few_shot)
    logger.info(f"Dataset size: train -> {len(train_dataset):d} ;")
files = args.tgt_files if args.tgt_files else args.files
dev_dataset = Example.load_dataset('valid', files, args.domains, fine_tuning=args.fine_tuning, few_shot=args.few_shot)
